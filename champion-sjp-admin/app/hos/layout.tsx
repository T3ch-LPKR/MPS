    ...
    <div className="min-h-screen bg-[#c9ccd2] flex justify-center">
      ...
      <div className="w-full max-w-[460px] bg-[#eef0f3] min-h-screen flex flex-col relative">
        ...
        <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[460px] bg-white border-t border-line flex">
          ...
          <Link href="/hos/berita" className={`flex-1 py-2 text-[11px] font-semibold flex flex-col items-center ${unread ? "text-brand" : "text-mut"}`}>
            <span className="relative inline-block leading-none">
              <span className="text-lg leading-none">📣</span>
              {unread ? <span className="absolute -top-1 -right-2 min-w-[15px] h-[15px] px-1 bg-brand text-white text-[9px] font-bold leading-[15px] rounded-full text-center">{unread > 9 ? "9+" : unread}</span> : null}
            </span>
            <span>Berita</span>
          </Link>
        </nav>
