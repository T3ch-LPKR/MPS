...
export default async function UsersPage() {
  const users = await q<any>(`SELECT user_id, username, full_name, role, emp_id, is_active, last_login FROM sjp_user_login ORDER BY username`);
  ...
  return (
    <>
      ...
      <div className="text-sm text-mut mb-5">Akun akses SJP. Password disimpan ter-enkripsi (bcrypt).</div>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.user_id} className="hover:bg-[#fafafa]">
                  ...
                  <td className="td"><span className="pill p-info uppercase">{u.role}</span>{u.emp_id ? <span className="text-[11px] text-mut ml-1">{u.emp_id}</span> : null}</td>
        ...
        <div className="card p-5 self-start">
          ...
          <form action={addUser} className="space-y-3">
            ...
            <div><label className="lbl">Role</label>
              <select name="role" className="inp"><option value="admin">Admin</option><option value="hos">Head of Sales</option><option value="salesman">Salesman</option></select></div>
            <div><label className="lbl">Link Salesman (opsional, utk role salesman)</label>
            ...
            <p className="text-[11px] text-mut">Password langsung di-hash bcrypt di database.</p>
          </form>
        </div>
      </div>
    </>
